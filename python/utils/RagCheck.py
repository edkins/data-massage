...
class RAGCheck:
    ...
    def get_top_chunks(self, question, chunks, top_k=5):
        """
    ...
    def rag_check(self, csv_string, top_k=5):
        ...
        for index, row in df.iterrows():
            ...
            if all_chunks:
                ...
                prompt = f"Check this question and answer to see if it is true. Here is the question: {question} Here are the answer choices and the selected answer: {row['choices']} Answer: {row['answer  ']} (0 indexed) Here is some context: {' '.join(top_chunks)} Return 0 if the answer is incorrect and 1 if the answer is correct. ONLY RETURN 0 or 1.  DO NOT RETURN ANYTHING ELSE"
                rag_result = call_claude(prompt)
                ...
                model_result = call_claude(prompt)
